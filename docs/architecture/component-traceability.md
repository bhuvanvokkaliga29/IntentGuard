...
| **Immutable Audit Ledger** | [`backend/db.py`](../../backend/db.py) | `create_audit_log()`, `create_decision()` | `GET /audit/{decision_id}` | `AuditLogRow` (`audit_logs`) | [`test_dataset_leakage.py`](../../backend/tests/test_dataset_leakage.py) |
